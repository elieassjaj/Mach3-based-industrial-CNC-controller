...
**Rule enforced by the Motion Engine (M5/M7/M8, `Docs/FIRMWARE-IMPLEMENTATION-PLAN.md`) — corrected by quantitative review (see the timing-verification note below):** a direction change for an axis may only take effect at a **STEP-DMA buffer segment boundary** (ADR-008's half/full-transfer refill points), and the axis must have **no remaining old-direction STEP edges anywhere in the segment currently playing** at the moment the new `GPIOD` value is written. The DIR write happens synchronized to that boundary (inside/at the start of the refill handling for the segment that will carry the new direction's first edge), not as an arbitrary, unsynchronized write from elsewhere.

**Why "one tick" was an incomplete description of the mechanism (not a wrong conclusion):** the STEP-DMA buffer is filled by the CPU *ahead* of when DMA actually plays it out (that is the entire point of buffering, ADR-008). A `GPIOD` write executed at the moment the CPU *computes* a future segment would flip the physical pin immediately, while DMA is still autonomously outputting the *previous*, still-playing segment's old-direction edges — a real DIR/STEP mismatch at the drive (exactly what `Docs/MOTION-ENGINE.md` Section 24 prohibits), not merely a tight margin. Restricting direction changes to segment boundaries and requiring the old direction's last edge to have already left the pipeline avoids this entirely. The practical consequence: **the real minimum lead time for a direction reversal is one STEP-DMA buffer refill period, not one base tick** — at any plausible buffer size (ADR-008, still TBD, but on the order of tens of microseconds for a reasonably sized buffer) this is 100–1,000× the 200 ns requirement, so the fixed timing spec remains satisfied with large margin; it only means a reversal costs up to one refill period of latency before it physically happens, which is negligible next to real mechanical deceleration times (milliseconds) but must be a known constraint for M8 (Planner) when it schedules reversals, and directly couples ADR-006 to ADR-008's (still open) buffer-depth choice: a smaller buffer reduces reversal latency at the cost of higher refill-interrupt frequency and CPU overhead.

**Timing impact:** Meets the fixed 200 ns setup/hold requirement with large margin (the achieved lead time is a buffer-refill period, not merely one tick); the original "1 tick exceeds 200 ns" arithmetic remains a valid — if conservative — lower-bound proof that compliance is achievable in principle. Exact hold/setup margin must still be confirmed on real hardware (Section 39), since GPIO output slew and DMA/CPU bus arbitration jitter both apply.

**Memory impact:** None beyond existing per-axis state.

**CPU impact:** Negligible — a plain `GPIOD->BSRR` write per direction change, at a segment-boundary refill event, not per tick.

**Risks:** If a future motion profile requires direction reversals more frequently than one buffer-refill period can absorb (e.g. extremely high-frequency oscillatory motion), either the buffer must be shrunk (trading CPU/refill overhead) or this decision should be revisited in favor of the DMA fallback above, which is not subject to this pipeline-depth constraint since it would be driven from the same tick-synchronous hardware path as STEP itself.
...
1. **STEP-DMA buffer (M4, low level):** a small, fixed-size circular buffer of precomputed `GPIOA->BSRR` words, refilled by the CPU on the DMA half-transfer and transfer-complete interrupts (already established in ADR-004's DMA configuration: circular mode, both interrupts enabled). Its size is an internal implementation parameter, not a protocol concern, and is not frozen by this ADR. Three factors trade off against each other when it is chosen (Section 39): keeping the refill-interrupt rate low (comfortably sub-10 kHz) and the per-refill CPU cost bounded, not costing meaningful RAM (a few KB is more than sufficient at 4 MHz), and — per ADR-006's quantitative timing review — **DIR reversal latency, which is bounded by exactly one refill period of this buffer**, so a smaller buffer here directly buys lower reversal latency at the cost of a higher refill-interrupt rate.
