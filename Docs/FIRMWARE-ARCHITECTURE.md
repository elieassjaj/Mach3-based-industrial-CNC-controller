...
2. **Play-time** (ISR at `t=iτ`, i.e., the *next* half-boundary — the one that fires exactly when `H(i)` starts playing, while it fills `H(i+1)`): as the **first action of this ISR, before any other work**, the code checks every axis's `armed_direction`; for any axis armed for `H(i)`, it writes the new value to `GPIOD->BSRR` now — synchronized to the real DMA event, since this ISR fires in hardware lock-step with `H(i)`'s playback start — then clears the armed entry.
...
**Setup margin — why a leading guard `g` (in ticks, within the target half) is still required beyond the half-boundary itself:** the play-time ISR's `GPIOD` write happens at `t = iτ + L`, where `L` is real Cortex-M4 interrupt-entry-plus-instruction latency (architecturally on the order of a few tens of ns at minimum; the actual worst case for this project is unmeasured and must be confirmed in Section 39). If `H(i)`'s very first tick (at `t=iτ`) already carried a new-direction STEP edge, the edge could fire before or concurrently with the still-settling `GPIOD` write — a real setup-time race, not merely a tight margin. The fix: reserve the first `g` ticks of `H(i)` as idle for axis X (already required at fill-time, above), so the first new-direction edge occurs at `t = iτ + g×250 ns`, giving a setup window of `g×250 ns − L` before that edge. Requiring `g×250 ns − L ≥ 200 ns`: with `L` conservatively bounded at up to ~250 ns pending real measurement, **`g = 2` (500 ns window) is the minimum viable value, `g = 3` (750 ns) is the recommended provisional default for comfortable margin** — both to be confirmed once `L` is actually measured (Section 39).

**Hold margin — no extra guard needed, by construction:** `GPIOD` is never touched between `H(i−1)`'s last tick and the play-time ISR at `t=iτ`, so the old direction is already stable for at least one full tick period (250 ns > 200 ns) — the natural gap between "the last old-direction edge, which occurs no later than `H(i−1)`'s final tick at `t=iτ−250 ns`" and "the write, which occurs at `t≥iτ`" — before the new value is written. No axis "parking" or extra trailing reserved ticks are needed on the hold side; the two-stage fill/play mechanism provides it for free.
...
**Timing impact:** Meets the fixed 200 ns setup/hold requirement, with the precise mechanism and margins (`g` ticks setup, free hold, `[N/2, N]×250 ns` reversal latency) derived above. Exact `L` (interrupt-entry latency) and the resulting minimum viable `g` must be confirmed on real hardware (Section 39).
