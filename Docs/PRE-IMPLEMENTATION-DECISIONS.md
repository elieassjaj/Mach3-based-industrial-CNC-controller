...
- **Resolved for bench testing — ADR-011, now implemented:** a locally-administered unicast address (U/L bit set), `02:00:05:10:00:01` — chosen only as a memorable convention echoing the static IP (`192.168.5.10`), with no other significance. `Firmware/LWIP/Target/ethernetif.c`'s `low_level_init()` now sets this value; it replaces the CubeMX placeholder `00:80:E1:00:00:00`, which is a real vendor's OUI prefix and must not be used even for bench testing on an isolated link. This literal is not driven by any `.ioc` field (CubeMX's LwIP template hardcodes it directly in generated code), so a future CubeMX regeneration will not silently revert it — but it is worth re-checking after any regeneration that touches `ethernetif.c`.
...
- **Needs your decision:** no — value confirmed, and the firmware is now written (Phase 2). `net_port_phy_hw_reset()` in `Firmware/Platform/STM32F407/Src/net_port_stm32f4.c` drives `PB0` low for 150 µs using a `DWT->CYCCNT` busy-wait that enables the counter itself if the stepgen port has not yet run, then releases and waits a 1 ms settle before returning. It is called from `low_level_init()`'s `USER CODE BEGIN MACADDRESS` block — before `HAL_ETH_Init()`, so the MAC's DMA soft reset never runs against a PHY still in reset. The 150 µs figure was re-checked against the datasheet in the repository while implementing: `LAN8720A_DataSheet-DS00002165.pdf` Table 5-9 gives `trstia` = 100 µs min exactly, so the adopted value sits at 1.5×. Hardware confirmation is HV-20.

- **Correction (prototype review).** "No RC delay, a plain pull-up" describes the Waveshare module wrongly: it has 4.7 kΩ + 100 nF on `nRST`, and `nRST` is **not on its header**, so on the prototype `PB0` reaches nothing. The decision holds for the **final PCB**, where `nRST` is wired to `PB0`, but its release must also satisfy `tpurstd` (≥ 25 ms after power-up), which the current firmware does not yet do. See `Docs/PROTOTYPE-BOARD.md` §5 and §7.
...
- **Needs your decision:** no — confirmed, already satisfied, and the recommended hardening is now **implemented**: `HV-21` (`Firmware/Platform/STM32F407/Src/net_selftest_stm32f4.c`) is the runtime check symmetric to HV-03, asserting that `DMARxDscrTab`, `DMATxDscrTab` and the `RX_POOL` pool all fall inside the SRAM1 range, and reporting the offending address when one does not. A future regression (e.g. a well-meaning but wrong `__attribute__((section(".ccmram")))` on an Ethernet buffer) now fails a test instead of failing silently on hardware. Re-confirmed statically in the Phase 2 verification build: `DMATxDscrTab` at `0x20000438`, `DMARxDscrTab` at `0x200004D8`, `memp_memory_RX_POOL_base` at `0x20000580` (0x4983 bytes) — all inside SRAM1, with `.stepgen_ram` still at `0x2001C000`–`0x2001D000` in SRAM2.
...
- **Needs your decision:** no — confirmed, already satisfied, and now checked at runtime. This is a binding invariant, not an incidental CubeMX default: any future change to either interrupt's priority (e.g. CubeMX regeneration resetting a value) must preserve `priority(ETH_IRQn), priority(ETH_WKUP_IRQn) > priority(DMA2_Stream1_IRQn)`. `HV-22` (`Firmware/Platform/STM32F407/Src/net_selftest_stm32f4.c`) reads all three priorities from the NVIC itself — not from the `.ioc`, which is what a regeneration rewrites — and fails if the ordering is ever lost. The same relationship is additionally enforced at compile time between `NET_PRIO_ETH` and `NET_PRIO_STEP_DMA` in `Firmware/Net/Inc/net_config.h`.
