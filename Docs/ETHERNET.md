...
> **Correction (prototype review).** The paragraph above rests on two premises that the LAN8720 module's own schematic contradicts. **`nRST` is not brought out to the module's header**; pin 13 is unconnected. And it is **not** a plain pull-up: C7 (100 nF) to GND makes it an RC reset that releases about 0.4 ms after power-up, about 60× short of `tpurstd`. On the prototype, therefore, the `PB0` pulse below reaches nothing, and the PHY is brought to a known state by `LAN8742_Init()`'s MDIO soft reset. ADR-013 remains the design for the **final PCB**, where `nRST` is wired to `PB0`, but its release must move to ≥ 25 ms after power-up to meet `tpurstd`. Full analysis: `Docs/PROTOTYPE-BOARD.md` §5 and §7.
