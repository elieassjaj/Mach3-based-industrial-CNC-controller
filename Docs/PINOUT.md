...
* **RELAY_PIN:** `PB8` (`GPIO_PIN_8`)
  * **Polarity: `[TBD — HARDWARE VERIFICATION REQUIRED]`.** Unlike every other output pin in this document (STEP/DIR/EN/spindle all have a stated polarity), the relay's active-high/active-low sense has never been documented, and no schematic for the relay driver circuit exists in this repository (`LAN8720A/LAN8720-ETH-Board-Schematic.pdf` covers only the Ethernet PHY module, not the relay stage). This must be read from the actual relay driver circuit (e.g. NPN/MOSFET low-side switch vs. high-side, opto-isolator orientation) before `IO/Outputs` (`Docs/FIRMWARE-IMPLEMENTATION-PLAN.md` M9) is implemented — it must not be guessed, since driving it backwards means the relay is energized whenever firmware intends it off (including at boot/fault, when it must default off).
